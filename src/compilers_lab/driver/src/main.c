...
static void print_compact_trace(const parser_stack *stack, 
                                 ...
                                 const grammar *g)
{
    ...
    if (action->type == PARSER_ACTION_SHIFT)
        printf(" | SHIFT %d\n", action->value);
    else if (action->type == PARSER_ACTION_REDUCE)
        printf(" | REDUCE %d\n", action->value);
    else if (action->type == PARSER_ACTION_ACCEPT)
        printf(" | ACCEPT\n");
}
 ...
 */
static bool parse_token_stream_with_trace(const grammar *g, const parser_table *table)
{
    ...
    while (true)
    {
        ...
        print_compact_trace(&stack, &lookahead, &action, g);
...
/**
 * @brief Program entry point. Builds or loads LALR table and parses yylex token stream.
...
int main(int argc, char **argv)
{
    ...
    const char *table_load_path = NULL;
    ...
    while ((opt = getopt(argc, argv, "l:")) != -1)
    {
        ...
        {
        case 'l':
            table_load_path = optarg;
            break;
        default:
            fprintf(stderr, "Usage: %s [-l table.csv] <grammar_file> [source_file] [table_output.(csv|json)]\n", argv[0]);
            return 1;
        }
    }
    ...
    grammar_path = argv[optind];
    
    if (remaining >= 2)
    {
        const char *second = argv[optind + 1];
        if (has_suffix(second, ".csv") || has_suffix(second, ".json"))
        {
            table_output_path = second;
        }
        else
        {
            source_path = second;
    ...
    if (remaining >= 3)
    {
        table_output_path = argv[optind + 2];
    }
    ...
    // load grammar
    char *grammar_file_content = read_file_all(grammar_path);
    if (grammar_file_content == NULL)
    {
        fprintf(stderr, "Failed to read grammar file '%s': %s\n", grammar_path, strerror(errno));
        return 1;
    }
    ...
    parser_table *table = NULL;
    
    if (table_load_path != NULL)
    {
        // load table from file
        printf("Loading parsing table from '%s'...\n", table_load_path);
        table = load_parser_table_csv(table_load_path, g);
        if (table == NULL)
        {
            fprintf(stderr, "Failed to load parser table from '%s'\n", table_load_path);
            return 1;
        }
        printf("Parsing table loaded (%d states, %d conflicts).\n", 
               table->num_states, table->num_conflicts);
    }
    else
    {
        // build as usual
        ...
        table = build_lalr1_parser_table(g, automaton);
        if (table == NULL)
        {
        ...
        {
            fprintf(stderr, "Failed to save parsing table to '%s'.\n", table_output_path);
            free_parser_table(table);
            free_lalr1_automaton(automaton);
            if (source_path != NULL && yyin != NULL) fclose(yyin);
            return 1;
        }
    ...
    bool accepted = parse_token_stream_with_trace(g, table);
